...
```bash
npm install @shawnma/pi-telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@shawnma/pi-telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@shawnma/pi-telemetry';
...
`@shawnma/pi-telemetry/testing` exports a runner-independent conformance suite modeled as grouped cases. A fixture supplies a fresh context and converts its backend's finished spans into normalized `RecordedTelemetrySpan` snapshots:

```typescript
import {
  createTelemetryAdapterConformance,
  type TelemetryAdapterFixture,
} from '@shawnma/pi-telemetry/testing';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@shawnma/pi-agent-core';
...
- `@shawnma/pi-telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@shawnma/pi-ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@shawnma/pi-agent-core` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
