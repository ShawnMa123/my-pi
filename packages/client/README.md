...
```ts
import { PiClient, type ByteTransportFactory } from "@shawnma/pi-client";
...
```ts
import { PiClient } from "@shawnma/pi-client";
import { createUnixTransportFactory } from "@shawnma/pi-client/unix";
...
The `@shawnma/pi-client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@shawnma/pi-client/unix` subpath.
