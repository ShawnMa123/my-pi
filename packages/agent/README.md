...
Stateful agent with tool execution and event streaming. Built on `@shawnma/pi-ai`.

## Installation

```bash
npm install @shawnma/pi-agent-core
```

### SQLite session backends

The SQLite session backend and the `node:sqlite` adapter live in a separate package, `@shawnma/pi-session-backend-sqlite-node`, so the core package does not pull in runtime builtins or native SQLite dependencies by default. The backend accepts a runtime-specific SQLite factory, allowing other session backends to ship as their own packages in the future.
...
```typescript
declare module "@shawnma/pi-agent-core" {
...
```typescript
import { Agent, streamProxy } from "@shawnma/pi-agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@shawnma/pi-agent-core";
