...
```bash
npm install @shawnma/pi-ai
```

TypeBox exports are re-exported from `@shawnma/pi-ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@shawnma/pi-ai';
import { builtinModels } from '@shawnma/pi-ai/providers/all';
...
import { openaiProvider } from '@shawnma/pi-ai/providers/openai';
import { openrouterProvider } from '@shawnma/pi-ai/providers/openrouter';
import { amazonBedrockProvider } from '@shawnma/pi-ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@shawnma/pi-ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@shawnma/pi-ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@shawnma/pi-ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@shawnma/pi-ai';
...
```typescript
import { validateToolCall, type Tool } from '@shawnma/pi-ai';
...
```typescript
import { builtinImagesModels } from '@shawnma/pi-ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@shawnma/pi-ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@shawnma/pi-ai';
import { openAICompletionsApi } from '@shawnma/pi-ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@shawnma/pi-ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@shawnma/pi-ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@shawnma/pi-ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@shawnma/pi-ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@shawnma/pi-ai';
import { anthropicProvider } from '@shawnma/pi-ai/providers/anthropic';
import { openaiProvider } from '@shawnma/pi-ai/providers/openai';
import { googleProvider } from '@shawnma/pi-ai/providers/google';
...
- `@shawnma/pi-ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@shawnma/pi-ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@shawnma/pi-ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@shawnma/pi-ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@shawnma/pi-ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@shawnma/pi-ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@shawnma/pi-ai/bedrock-provider';
...
```bash
npx @shawnma/pi-ai login              # interactive provider selection
npx @shawnma/pi-ai login anthropic    # login to specific provider
npx @shawnma/pi-ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@shawnma/pi-ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@shawnma/pi-ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@shawnma/pi-ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@shawnma/pi-ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@shawnma/pi-ai`.
