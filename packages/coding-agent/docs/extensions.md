...
| Package | Purpose |
|---------|---------|
| `@shawnma/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@shawnma/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@shawnma/pi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@shawnma/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@shawnma/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@shawnma/pi-coding-agent";
...
```typescript
import { CONFIG_DIR_NAME, type ExtensionAPI } from "@shawnma/pi-coding-agent";
...
```typescript
import { SessionManager } from "@shawnma/pi-coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@shawnma/pi-ai";
...
```typescript
import type { AutocompleteItem } from "@shawnma/pi-tui";
...
```typescript
import { Box, Text } from "@shawnma/pi-tui";
...
Extensions that need native provider auth, filtering, refresh, or stream behavior can register a complete `Provider` from `@shawnma/pi-ai`. The provider becomes the composition base and `models.json` overrides still apply above it.

```typescript
import { createProvider, openAICompletionsApi } from "@shawnma/pi-ai";
...
```typescript
import { withFileMutationQueue } from "@shawnma/pi-coding-agent";
...
import { Type } from "typebox";
import { StringEnum } from "@shawnma/pi-ai";
import { Text } from "@shawnma/pi-tui";
...
**Important:** Use `StringEnum` from `@shawnma/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@shawnma/pi-coding-agent";
...
```typescript
import { createBashTool } from "@shawnma/pi-coding-agent";
...
```typescript
import { keyHint } from "@shawnma/pi-coding-agent";
...
```typescript
import { Text, Component } from "@shawnma/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@shawnma/pi-coding-agent";
import { matchesKey } from "@shawnma/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@shawnma/pi-coding-agent";
