...
**Source:** [`@shawnma/pi-tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@shawnma/pi-tui";
...
```typescript
import { Container, type Focusable, Input } from "@shawnma/pi-tui";
...
Import from `@shawnma/pi-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@shawnma/pi-tui";
```
...
```typescript
import { matchesKey, Key } from "@shawnma/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@shawnma/pi-tui";
...
```typescript
import { getMarkdownTheme } from "@shawnma/pi-coding-agent";
import { Markdown } from "@shawnma/pi-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@shawnma/pi-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@shawnma/pi-coding-agent";
import { DynamicBorder } from "@shawnma/pi-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@shawnma/pi-tui";
...
```typescript
import { BorderedLoader } from "@shawnma/pi-coding-agent";
...
For toggling multiple settings. Use `SettingsList` from `@shawnma/pi-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@shawnma/pi-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@shawnma/pi-tui";
...
import { matchesKey, truncateToWidth } from "@shawnma/pi-tui";
