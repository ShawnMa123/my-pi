...
| Variable | Description |
|----------|-------------|
| `MY_PI_CODING_AGENT_DIR` | Override the config directory; default is `~/.my-pi/agent` (derived from `piConfig.name`) |
| `MY_PI_CODING_AGENT_SESSION_DIR` | Override session storage; overridden by `--session-dir` |
