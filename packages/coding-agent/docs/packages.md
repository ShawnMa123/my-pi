...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@shawnma/pi-ai`, `@shawnma/pi-agent-core`, `@shawnma/pi-coding-agent`, `@shawnma/pi-tui`, `typebox`.
