...
describe("detectInstallMethod", () => {
	test("detects pnpm from Windows .pnpm install paths", () => {
		...
		expect(getUpdateInstruction("@shawnma/pi-coding-agent")).toBe(
			"Run: pnpm install -g --ignore-scripts --config.minimumReleaseAge=0 @shawnma/pi-coding-agent",
		);
	});
	...
	test("does not self-update unknown wrapper installs", () => {
		...
		expect(getSelfUpdateCommand("@shawnma/pi-coding-agent")).toBeUndefined();
		expect(getUpdateInstruction("@shawnma/pi-coding-agent")).toBe(
			"Update @shawnma/pi-coding-agent using the package manager, wrapper, or source checkout that provides this installation.",
		);
	});
	...
	test("self-updates npm installs from custom prefixes", () => {
		...
		const command = getSelfUpdateCommand("@shawnma/pi-coding-agent");
		...
		expect(command).toEqual({
			...
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @shawnma/pi-coding-agent`,
		});
	});
	...
	test("self-updates exact npm versions without uninstalling the current package", () => {
		...
		const command = getSelfUpdateCommand("@shawnma/pi-coding-agent", undefined, {
			packageName: "@shawnma/pi-coding-agent",
			installSpec: "@shawnma/pi-coding-agent@1.2.3",
		});
		...
		expect(command).toEqual({
			...
			args: [
				...
				"--ignore-scripts",
				"--min-release-age=0",
				"@shawnma/pi-coding-agent@1.2.3",
			],
			display: `npm --prefix ${prefix} install -g --ignore-scripts --min-release-age=0 @shawnma/pi-coding-agent@1.2.3`,
		});
	});
	...
	test("self-update respects configured npmCommand", () => {
		...
		const command = getSelfUpdateCommand("@shawnma/pi-coding-agent", ["npm", "--prefix", prefix]);
	...
	test("quotes npm self-update display paths", () => {
		...
		expect(command?.display).toBe(
			`npm --prefix "${prefix}" install -g --ignore-scripts --min-release-age=0 @shawnma/pi-coding-agent`,
		);
	});
	...
	test("does not infer Windows npm custom prefixes from package paths", () => {
		...
		expect(detectInstallMethod()).toBe("npm");
		expect(getUpdateInstruction("@shawnma/pi-coding-agent")).toBe(
			"Run: npm install -g --ignore-scripts --min-release-age=0 @shawnma/pi-coding-agent",
		);
	});
	...
	test("self-updates bun global installs from bun pm bin", () => {
		...
		expect(command).toEqual({
			command: "bun",
			args: ["install", "-g", "--ignore-scripts", "--minimum-release-age=0", "@shawnma/pi-coding-agent"],
			display: "bun install -g --ignore-scripts --minimum-release-age=0 @shawnma/pi-coding-agent",
		});
	});
	...
	test("self-updates pnpm v11 global installs resolved through the store", () => {
		...
		const packageName = "@shawnma/pi-coding-agent";
	...
	test("does not self-update when npm install path is not writable", () => {
		...
		expect(getSelfUpdateCommand("@shawnma/pi-coding-agent")).toBeUndefined();
		expect(getSelfUpdateUnavailableInstruction("@shawnma/pi-coding-agent")).toContain(
